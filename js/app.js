    ...
    createPostBtn.addEventListener("click", async () => {
      const { value: formValues } = await Swal.fire({
        ...
        html: `
            ...
            <label>Choose Technology:</label><br>
            <label>
              <input type="radio" name="technology" value="Design Thinking" /> Design Thinking
            </label><br>
            <label>
              <input type="radio" name="technology" value="AI" /> AI
            </label><br>
            <label>
              <input type="radio" name="technology" value="Web3" /> Web3
            </label><br>
            <label>
              <input type="radio" name="technology" value="Programming" /> Programming
            </label>
          </div>
...
async function displayPosts(searchQuery = "") {
    ...
    try {
      ...
      querySnapshot.forEach((doc) => {
        ...
        // Check if the post matches the search query (in title, description, or technology)
        const isMatch = (
          post.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
          post.description.toLowerCase().includes(searchQuery.toLowerCase()) ||
          post.technology.toLowerCase().includes(searchQuery.toLowerCase())
        );
        ...
        if (isMatch) {
            ...
            <div class="card mb-3" data-id="${postId}">
              <div class="card-body">
                ...
                <p class="text-muted small">
                  ${post.timestamp ? new Date(post.timestamp.toMillis()).toLocaleString() : "No timestamp"}
                </p>
                <p class="font-weight-bold">${post.username}</p>
                <p class="badge badge-info">${post.technology}</p> <!-- Display the technology -->
          ...
          `;
          postsContainer.innerHTML += postCard; // Append post card
        }
      });
    } catch (error) {
      showError(error);
